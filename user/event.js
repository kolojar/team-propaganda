...
for (const element of document.getElementsByClassName("siteInfo")) {
    SetupSaveCancelButtons(element, ".", "./site.php", element.getAttribute("site"));
}
for (const element of document.getElementsByClassName("presInfo")) {
    SetupSaveCancelButtons(element, ".", "./pres.php", element.getAttribute("site"));
}
...
btnPay === null || btnPay === void 0 ? void 0 : btnPay.addEventListener("click", async () => {
    ...
    //Get bank account
    const bankAccount = await GlobalDialogManager.ShowPromptAsync("Zaplatit", "Zadejte číslo účtu pro případné vrácení peněz.", null, "text", { placeholder: "Číslo účtu" });
    if (bankAccount == null) {
        SendToast("Zaplatit", "Zadání platby bylo zrušeno.", "info");
        return;
    }
    //Show info for payment
    if (!await GlobalDialogManager.ShowConfirmAsync("Odešlete prosím platbu na následující účet", "Číslo účtu: <span class='fontMono'>" + responce + "</span><br>Variabilní symbol: <span class='fontMono'>" + responce3 + "</span><br>Částka: <span class='fontMono'>" + (btnPay === null || btnPay === void 0 ? void 0 : btnPay.getAttribute("price")) + "</span> Kč<br>Číslo účtu pro případné vrácení peněz: <span class='fontMono'>" + bankAccount + "</span>", { allowSelect: true })) {
...
(_a = document.getElementById("btnRemoveAttendant")) === null || _a === void 0 ? void 0 : _a.addEventListener("click", async () => {
    //Get confirmation
    const reason = await GlobalDialogManager.ShowPromptAsync("Odhlásit zájemce z akce?", "Opravdu chcete odhlásit zájemce z akce? Tento krok nelze vzít zpět. Sdělte nám prosím důvod.", null, "textarea", { placeholder: "Zadejte důvod odhlášení" });
    if (reason == null) {
...
(_b = document.getElementById("btnRemoveCD")) === null || _b === void 0 ? void 0 : _b.addEventListener("click", async (e) => {
    if (!await GlobalDialogManager.ShowConfirmAsync("Odhlásit firmu ze dne firem?", "Opravdu chcete odhlásit Vaši firmu z tohoto dne firem?"))
        return;
    let data = new FormData();
...
(_c = document.getElementById("btnAddSite")) === null || _c === void 0 ? void 0 : _c.addEventListener("click", async (e) => {
    if (!await GlobalDialogManager.ShowConfirmAsync("Přidat nový stánek?", "Opravdu chcete přidat nový stánek pro tento dne firem?"))
        return;
    let data = new FormData();
...
(_d = document.getElementById("btnAddPres")) === null || _d === void 0 ? void 0 : _d.addEventListener("click", async (e) => {
    if (!await GlobalDialogManager.ShowConfirmAsync("Přidat novou prezentaci?", "Opravdu chcete přidat novou prezentaci pro tento dne firem?"))
        return;
    let data = new FormData();
