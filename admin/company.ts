...
manageFieldsBtn?.addEventListener("click", async () => {
	//Get fields from HTML
	const fieldsJson = JSON.parse(manageFieldsBtn.getAttribute("fields") as string);
	const fieldsSelectedJson = JSON.parse(manageFieldsBtn.getAttribute("checked-fields") as string);
	const fields = new Map<string, FormDialogCheckboxSelectData<string>>();
	...
	data.set("id", urlSearchParams.get("company") as string);
